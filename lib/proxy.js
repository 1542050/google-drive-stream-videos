...
* @Last Modified time: 2017-03-09 12:44:29
*/
...
const toProxyURL = (url, cookie) => {
  var hash = base64(JSON.stringify({
    cookie,
    domain: url.split('/kimdepzai?')[0]
  }))
  return `http://${process.env.VIRTUAL_HOST}:${process.env.PORT}/kimdepzai?hash=${hash}&` + url.split('?').pop()
}
